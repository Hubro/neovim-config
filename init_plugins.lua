...
require("lazy").setup({
  ...
  -- Copilot (Remember to run ":Copilot setup" to sign in)
  {
    ...
    init = function()
      ...
      local _plug_accept = function(thing)
        local expr = vim.api.nvim_replace_termcodes(
          "<Plug>(copilot-accept-" .. thing .. ")", true, true, true
        )

        vim.api.nvim_feedkeys(expr, "m", false)
      end
      ...
      vim.keymap.set("i", "<A-l>", accept_word)
      vim.keymap.set("i", "<A-h>", accept_line)
      -- vim.keymap.set("i", "<A-l>", "<Plug>(copilot-accept-word)", { remap = true })
      -- vim.keymap.set("i", "<A-h>", "<Plug>(copilot-accept-line)", { remap = true })
